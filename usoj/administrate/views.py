...
def rejudge(request):
    if request.method == 'POST':
        ...
        if retype == 'solution':
            ...
            res = { 'lang' : sol.languge, 'id' : problem.id, 'timelim' : problem.time_limit, 'memorylim' : problem.memory_limit, 'code' : sol.code}
            ...
            for eachsol in sol:
                res = { 'lang' : eachsol.languge, 'id' : problem.id, 'timelim' : problem.time_limit, 'memorylim' : problem.memory_limit, 'code' : eachsol.code}
...
def showProblem(request):
    ...
    return render(request, 'manage/showproblem.html', {'problems' : problems})
...
def showContest(request):
    ...
    return render(request, 'manage/showcontest.html', {'contests' : contests, 'problems':problem, 'users':user, 'groups':group})
...
def showJudger(request):
    ...
    return render(request, 'manage/showjudger.html', {'judgers':judgers})

@power_required(redirect='/', powerneed=100)
def showAccount(request):
    users = User.objects.all()
    return render(request, 'manage/showaccount.html', {'users' : users})
...
def addProblem(request):
    if request.method == 'POST':
        ...
        if not id:
            ...
            problem = Problem()
            problem.title = request.POST.get('title', 'No Title') if request.POST.get('title', 'No Title') != "" else 'No Title'
            problem.description = request.POST.get('description', "Description empty.") if request.POST.get('description', "Description empty.") != "" else 'Description empty.'
            ...
            problem.time_limit = request.POST.get('time_limit', 1) if request.POST.get('time_limit', 1) != "" else 1
            problem.memory_limit = request.POST.get('memory_limit', 128) if request.POST.get('memory_limit', 128) != "" else 128
            problem.source = request.POST.get('source', "")
            problem.difficulty = request.POST.get('difficulty', 0) if request.POST.get('difficulty', 0) != "" else 1
            ...
            problem.title = request.POST.get('title', problem.title) if request.POST.get('title', problem.title) != "" else problem.title
            problem.description = request.POST.get('description', problem.description)
            problem.input_description = request.POST.get('input_description', problem.input_description)
            problem.output_description = request.POST.get('output_description', problem.output_description)
            ...
            problem.time_limit = request.POST.get('time_limit', 1) if request.POST.get('time_limit', 1) != "" else problem.time_limit
            problem.memory_limit = request.POST.get('memory_limit', 128) if request.POST.get('memory_limit', 128) != "" else problem.time_limit
            problem.source = request.POST.get('source', "")
            problem.difficulty = request.POST.get('difficulty', 0) if request.POST.get('difficulty', 0) != "" else problem.time_limit
...
def addContest(request):
    if request.method == 'POST':
        ...
        if not id:
            contest = Contest()
            contest.title = request.POST.get('title', 'No Title') if request.POST.get('title', 'No Title') != "" else 'No Title'
            contest.description = request.POST.get('description', "Description empty.")
            contest.start_time = request.POST.get('start_time', timezone.now()) if request.POST.get('start_time', timezone.now()) != "" else timezone.now()
            contest.end_time = request.POST.get('end_time', timezone.now()) if request.POST.get('end_time', timezone.now()) != "" else timezone.now()
            contest.contest_type = request.POST.get('contest_type', "1") if request.POST.get('contest_type', "1") != "" else 1
            ...
            for each in problem:
                if each:
                    add = ContestProblem(show_id=str(showid), contest=contest, problem=Problem.objects.get(id=int(each)))
                    add.save()
                    showid += 1
            for each in user:
                if each:
            ...
            contest.title = request.POST.get('title', contest.title) if request.POST.get('title', 'No Title') != "" else 'No Title'
            contest.description = request.POST.get('description', contest.description)
            contest.start_time = request.POST.get('start_time', contest.start_time) if request.POST.get('start_time', timezone.now()) != "" else timezone.now()
            contest.end_time = request.POST.get('end_time', contest.end_time) if request.POST.get('end_time', timezone.now()) != "" else timezone.now()
            contest.contest_type = request.POST.get('contest_type', contest.contest_type) if request.POST.get('contest_type', "1") != "" else 1
            contest.password = request.POST.get('password', contest.password)
            contest.save()
            ...
            for each in problem:
                if each:
                    add = ContestProblem(show_id=0, contest=contest, problem=Problem.objects.get(id=int(each)))
                    add.save()
...
def addAccount(request):
    if request.method == 'POST':
        ...
        user.real_name = request.POST.get('realname', "") if request.POST.get('realname', "") != "" else user.real_name
        user.admin_type = request.POST.get('admin_type', 0) if request.POST.get('admin_type', "1") != "" else 0
...
def addJudger(request):
    if request.method == 'POST':
        ...
        newjudger = Judger(max=request.POST.get('max', 4), ip=request.POST.get('ip', '0.0.0.0'), port=request.POST.get('port', 8888), remote=remote, token=request.POST.get('token', ''))
        newjudger.save()
        return HttpResponseRedirect("/manage/judger/")

    return render(request, 'manage/addjudger.html', {'action' : 'add'})
...
def editJudger(request):
    ...
    remote = 1 if editjudger.remote == True else 0
    return render(request, 'manage/addjudger.html', {'action' : 'edit', 'max' : editjudger.max, 'ip' : editjudger.ip, 'port':editjudger.port, 'remote': remote, 'token' : editjudger.token, 'id' : editid})
...
def downloadtest(request):
    ...
    for dirpath, dirnames, filenames in os.walk('/home/problem/'+str(id)+'/'):
            ...
            try:
                ...
                for item in doc.iterfind('item'):
                    ...
                    addProblem = Problem(title=title, description=description, input_description=input, output_description=output, \
                                    ...
                                    visible=False, spj=False, source=source, difficulty=0, show_id=1, created_by=request.user)
                    addProblem.save()
